...
try {
	...
	let actucalreservation;
		...
		if (!Reservations) {
			...
			let chairsArray = Reservations.reservedSeats as number[] | null;
	...
	if (process.env.SENDGRID_API_KEY) {
		...
		let actucalreservation1;
			...
			if (!Reservations1) {
				...
				let chairsArray = Reservations1.reservedSeats as number[] | null;
			...
			if (!Reservations2) {
				...
				let chairsArray = Reservations2.reservedSeats as number[] | null;
...
export const paymentCallback = async (
	...
	try {
		...
		if(voucher.length > 0) {
		  const usedvoucher = await prisma.voucher.findUnique({where:{code:voucher}}) ;
		  const increasevoucher = await prisma.voucher.update({where:{code:voucher} , data : {consumed : {increment : 1}}}) ;
		  actualvoucher = usedvoucher ;
		}
...
export const paymobPay = async (
	...
	const formattedDate = `${year}-${month.padStart(2, "0")}-${day.padStart(2, "0")}`;
		...
		if(OldtempReservations) {
			const updatedReservedSeats = OldtempReservations.reservedSeats as number[] | null;
			...
			await prisma.tempReservation.update({
				...
				data: {
					reservedSeats_counter: OldtempReservations.reservedSeats_counter + ReservedChairs.length,
					reservedSeats: updatedReservedSeats || [],
					
				},
			});
		}else {
			const TempReserve = await prisma.tempReservation.create({data : {
				trip_id : trip_id ,
...
export const doublepaymobPay = async (
	...
	const formattedDate2 = `${year2}-${month2.padStart(2, "0")}-${day2.padStart(2, "0")}`;
		...
		if(OldtempReservations1) {
			const updatedReservedSeats = OldtempReservations1.reservedSeats as number[] | null;
			...
			await prisma.tempReservation.update({
				...
				data: {
					reservedSeats_counter: OldtempReservations1.reservedSeats_counter + ReservedChairs1.length,
					reservedSeats: updatedReservedSeats || [],
		...
		if(OldtempReservations2) {
			const updatedReservedSeats = OldtempReservations2.reservedSeats as number[] | null;
			...
			await prisma.tempReservation.update({
				...
				data: {
					reservedSeats_counter: OldtempReservations2.reservedSeats_counter + ReservedChairs2.length,
					reservedSeats: updatedReservedSeats || [],
